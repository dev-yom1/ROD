...
} from "../lib/orchestrator/ordered-diagnose-pr";
...
async function runDiagnosisStep(
  ...
  try {
    const { diagnosePullRequest } = await import("../lib/orchestrator/ordered-diagnose-pr");
...
async function finalizeFailureStep(
  ...
  const { failPullRequestDiagnosis } = await import("../lib/orchestrator/ordered-diagnose-pr");
