...
type ParsedFence = { index: number; startLine: number; endLine: number; steps: ReadmeStep[] };
type SectionBucket = {
  key: number;
  section: string | null;
  startLine: number;
...
function parseLogicalCommands(lines: Array<{ text: string; line: number }>): LogicalCommand[] {
  ...
  if (pendingParts.length) commands.push({ command: `${pendingParts.join(" ").trim()} \\`, line: pendingLine, malformed: true });
  return commands;
}

function portHints(command: string): number[] {
  const ports = new Set<number>();
  const patterns = [/(?:^|\s)--port(?:=|\s+)(\d{2,5})(?=\s|$)/gi, /(?:^|\s)-p\s+(\d{2,5})(?=\s|$)/gi, /(?:^|\s)PORT=(\d{2,5})(?=\s|$)/g];
...
function blockedShellSyntax(command: string): boolean {
  ...
  return [/\bcurl\b/i, /\bwget\b/i, /\bssh\b/i, /\bscp\b/i, /\bgit\s+push\b/i, /\bnpm\s+publish\b/i, /\b(?:vercel|netlify|flyctl)\s+(?:deploy|--prod)\b/i, /\bterraform\s+(?:apply|destroy)\b/i, /\bkubectl\b/i, /\b(?:aws|gcloud|az)\b/i, /\bsudo\b/i, /\brm\s+-rf\s+\/(?:\s|$)/i, /\b(?:docker|podman)\s+(?:push|login)\b/i].some((pattern) => pattern.test(command));
}
...
function wrappedNodeRole(body: string): ReadmeStepRole | null {
  ...
  if (framework === "vite") return subcommand === null || subcommand === "dev" || subcommand === "serve" || subcommand.startsWith("-") ? "start" : "other";
  return "other";
}
...
export function parseOnboardingCommand(command: string, malformed = false): ParsedOnboardingCommand {
  ...
  if (malformed || !command.trim() || blockedShellSyntax(command)) return { role: "other", safe: false, executable: false, runtime: null, envCopySource: null, portHints: hints };
  const body = stripLeadingEnvAssignments(command);
  const copy = body.match(ENV_COPY);
  if (copy) return { role: "preparation", safe: true, executable: true, runtime: null, envCopySource: copy[1].toLowerCase() as EnvTemplateName, portHints: hints };
  if (/^(?:mkdir|touch)\s+/i.test(body)) return { role: "preparation", safe: true, executable: true, runtime: null, envCopySource: null, portHints: hints };
  if (/^(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(?:dev|start)(?:\s|$)/i.test(body)) return { role: "start", safe: true, executable: true, runtime: "node", envCopySource: null, portHints: hints };
  const wrappedNode = wrappedNodeRole(body);
  if (wrappedNode) return { role: wrappedNode, safe: true, executable: true, runtime: "node", envCopySource: null, portHints: hints };
  if (nestedPythonStart(body)) return { role: "start", safe: true, executable: true, runtime: "python", envCopySource: null, portHints: hints };
  const wrappedPython = body.match(/^(?:poetry|uv)\s+run\s+(.+)$/i);
  if (wrappedPython && nestedPythonStart(wrappedPython[1])) return { role: "start", safe: true, executable: true, runtime: "python", envCopySource: null, portHints: hints };
  const nodeInstall = [/^npm\s+(?:ci|install|i)(?:\s|$)/i, /^pnpm\s+(?:install|i)(?:\s|$)/i, /^yarn\s*$/i, /^yarn\s+install(?:\s|$)/i, /^yarn\s+--(?:immutable|immutable-cache|check-cache|frozen-lockfile)(?:[=\s]|$)/i, /^bun\s+install(?:\s|$)/i].some((pattern) => pattern.test(body));
  if (nodeInstall) return { role: "install", safe: true, executable: true, runtime: "node", envCopySource: null, portHints: hints };
  const pythonInstall = [/^(?:python|python3)\s+-m\s+pip\s+install(?:\s|$)/i, /^pip(?:3)?\s+install(?:\s|$)/i, /^poetry\s+install(?:\s|$)/i, /^uv\s+sync(?:\s|$)/i].some((pattern) => pattern.test(body));
  if (pythonInstall) return { role: "install", safe: true, executable: true, runtime: "python", envCopySource: null, portHints: hints };
  if (/^(?:npm|pnpm|yarn|bun)(?:\s|$)/i.test(body)) return { role: "other", safe: true, executable: true, runtime: "node", envCopySource: null, portHints: hints };
  if (/^(?:poetry\s+run|uv\s+run)(?:\s|$)/i.test(body)) return { role: "other", safe: true, executable: true, runtime: "python", envCopySource: null, portHints: hints };
...
function parseSections(markdown: string): SectionBucket[] {
  ...
  let sectionKey = 0;
  let fenceIndex = 0;
  let current: SectionBucket = { key: sectionKey, section: null, startLine: 1, endLine: lines.length, lines: [], fences: [], steps: [] };
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (heading && heading[1].length <= 2) {
      ...
      sectionKey += 1;
      current = { key: sectionKey, section: heading[2].replace(/\s+#+\s*$/, "").trim(), startLine: index + 1, endLine: lines.length, lines: [{ line: index + 1, text: line }], fences: [], steps: [] };
      buckets.push(current);
      continue;
    }
    ...
    const steps = logical.map((item, occurrence): ReadmeStep => {
      ...
      return { id: `s${current.key}:f${thisFenceIndex}:l${item.line}:o${occurrence}`, command: item.command, section: current.section, fenceIndex: thisFenceIndex, line: item.line, role: parsed.role, malformed: item.malformed || undefined };
    });
...
function explicitVariantFamily(bucket: SectionBucket): string | null {
  ...
  for (const family of ["npm", "pnpm", "yarn", "bun", "pip", "poetry", "uv"]) {
    if (new RegExp(`(?:\\b(?:with|using|for)\\s+${family}\\b|\\b${family}\\s+(?:installation|install|setup)\\b|^(?:${family})$)`, "i").test(section)) return family;
  }
  return null;
}
function ambiguousInstallFamilies(bucket: SectionBucket): RuntimeKind | null {
  ...
  for (const [runtime, families] of byRuntime) if (families.size > 1) return runtime;
  return null;
}
...
function selectOnboardingFlow(markdown: string): SelectedFlow | null {
  ...
  const terminal = (startBuckets.length ? startBuckets : buckets).slice().sort((a, b) => {
    ...
    return score !== 0 ? score : a.startLine - b.startLine;
  })[0];
  ...
  if (!terminalStart) {
    const terminalSteps = terminal.fences.filter((fence) => fence.index <= terminalFence.index).flatMap((fence) => fence.steps);
    return { steps: terminalSteps, text: terminalText, terminalText, terminalAfterText, sections: terminal.section ? [terminal.section] : [], issue: null };
  }
  const setupCandidates = buckets.filter((bucket) => bucket.startLine < terminal.startLine && isSetupBucket(bucket));
  ...
  if (variantBuckets.length > 1) {
    ...
    if (matching.length === 1) selectedVariant = matching[0];
    else flowIssue = `ROD found multiple explicit setup variants before the selected start flow and could not choose exactly one for ${terminalFamily ?? "the start command"}.`;
  }
  const genericAmbiguity = setupCandidates.find((bucket) => explicitVariantFamily(bucket) === null && ambiguousInstallFamilies(bucket) !== null);
  if (genericAmbiguity) {
  ...
  if (startFences.length > 1) flowIssue = "ROD found start commands in multiple README shell fences inside the same terminal onboarding section and cannot safely infer which alternative path to execute.";
  else if (terminalStarts.length > 1) flowIssue = "ROD found multiple start commands in the same README shell fence and cannot safely infer which alternative onboarding path to execute.";
  const setupBuckets = setupCandidates.filter((bucket) => variantBuckets.length <= 1 || !variantBuckets.includes(bucket) || bucket === selectedVariant);
  const setupSteps = setupBuckets.flatMap((bucket) => bucket.steps);
  const terminalSteps = terminal.fences.filter((fence) => fence.index <= terminalFence.index).flatMap((fence) => fence.steps);
  const steps = [...setupSteps, ...terminalSteps];
  const textParts = [...setupBuckets.flatMap((bucket) => bucket.lines.map((item) => item.text)), ...terminalText.split("\n")];
  const sections = [...setupBuckets, terminal].map((bucket) => bucket.section).filter((section): section is string => Boolean(section));
...
function extractRuntime(markdown: string, runtime: "node" | "python"): string | null {
  ...
  const otherName = runtime === "node" ? /\bPython\b/i : /\bNode(?:\.js)?\b/i;
  const token = /(?:>=|<=|>|<|\^|~=|~|==|=)?\s*v?\d+(?:\.(?:\d+|x|\*)){0,2}\+?/gi;
  for (const line of markdown.split("\n")) {
    const runtimeMatch = name.exec(line);
    if (!runtimeMatch) continue;
    const rawTail = line.slice(runtimeMatch.index + runtimeMatch[0].length);
    const otherRuntime = otherName.exec(rawTail);
    const tail = otherRuntime ? rawTail.slice(0, otherRuntime.index) : rawTail;
    const matches = [...tail.matchAll(token)].map((match) => match[0].trim()).filter(Boolean);
    if (!matches.length) continue;
    if (/\b(?:or\s+(?:newer|later)|and\s+(?:newer|later))\b/i.test(tail) || /\+$/.test(matches[0])) return `>=${matches[0].replace(/\+$/, "").replace(/^[=\s]+/, "")}`;
    const bareList = matches.length > 1 && /,/.test(tail) && /\bor\b/i.test(tail) && matches.every((match) => /^v?\d+(?:\.(?:\d+|x|\*)){0,2}$/.test(match));
    if (bareList) return matches.map((match) => match.replace(/^v/i, "")).join(" || ");
    const alternatives = tail.split(/\s*(?:\|\||\bor\b)\s*/i);
    if (alternatives.length > 1) {
      const groups = alternatives.map((alternative) => [...alternative.matchAll(token)].map((match) => match[0].trim()).filter(Boolean).join(" "));
...
export function extractReadmePlan(markdown: string): ReadmePlan {
  ...
  return {
    ...
    expectedPort: urlMatch?.[1] ? Number(urlMatch[1]) : null,
    expectedUrl: urlMatch?.[0] ?? null,
...
export function isSafeOnboardingCommand(command: string): boolean { return parseOnboardingCommand(command).safe; }
