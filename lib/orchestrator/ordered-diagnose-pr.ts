...
import { runnerReadmePlan } from "../runner/readme-order";
...
export async function diagnosePullRequest(
  ...
  const sandboxPlan = runnerReadmePlan(plan);
  const sandboxFacts = plan.installCommand && !sandboxPlan.installCommand
    ? { ...initialFacts, inferredInstallCommand: null }
    : initialFacts;
  ...
  const sandboxResult = await runSandboxDiagnosis(archive, sandboxPlan, sandboxFacts);
